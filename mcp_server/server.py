...
async def get_user_by_id(user_id: int) -> str:
    """Get a user by their ID from the user management system"""
    return await user_client.get_user(user_id)


@mcp.tool()
async def delete_user(user_id: int) -> str:
    """Delete a user by their ID from the user management system"""
...
async def search_user(
    ...
    """Search for users in the user management system by name, surname, email, or gender. All parameters are optional and support partial matching."""
...
async def add_user(user_data: UserCreate) -> str:
    """Add a new user to the user management system with the provided user data"""
    return await user_client.add_user(user_data)


@mcp.tool()
async def update_user(user_id: int, user_data: UserUpdate) -> str:
    """Update an existing user in the user management system with the provided user data"""
...
async def get_flow_diagram() -> bytes:
    """Provides a flow diagram showing the architecture and endpoints of the User Management Service"""
...
async def search_guidance() -> str:
    """Provides guidance on how to effectively search through the user database with available parameters and strategies"""
    return """You are helping users search through a dynamic user database. The database contains realistic synthetic user profiles with the following searchable fields:
...
- "Find all Johns" uses name="john"
- "Gmail users named Smith" uses email="gmail" + surname="smith"
- "Female users with company emails" uses gender="female" + email="company"
- "Users with Johnson surname" uses surname="johnson"
...
When helping users search, suggest multiple search strategies and explain why certain approaches might be more effective for their goals."""


@mcp.prompt()
async def user_creation_guidance() -> str:
    """Provides comprehensive guidelines for creating realistic and valid user profiles with proper data formatting"""
    return """You are helping create realistic user profiles for the system. Follow these guidelines to ensure data consistency and realism.
...
Address Guidelines - Provide complete, realistic addresses:
...
Credit Card Guidelines - Generate realistic but non-functional card data:
...
Biography Templates - Use varied narrative structures:
